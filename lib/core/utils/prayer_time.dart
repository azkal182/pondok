...
class PrayerTimes {
  ...
  PrayerTimes() {
    ...
    // Kemenag
    List<double> Kevalues = [20, 0, 0, 0, 18];
    methodParams.addAll({getKemenag(): Kevalues});
  }
  ...
  double computeAsr(double step, double t) {
    double D = sunDeclination(getJDate() + t);
    double G = -darccot(step + dtan((getLat() - D).abs()));
  ...
  setCustomParams(List<double> params) {
    for (int i = 0; i < 5; i++) {
      if (params[i] == -1) {
        params[i] = mParams[i];
        methodParams.addAll({getCustom(): params});
      } else {
        mParams[i] = params[i];
      }
    }
...
//        hrs += 1;*/
  ...
  List<double> computeTimes(List<double> times) {
    ...
    double Fajr = computeTime(180.0 - x[0], t[0]);

    double Sunrise = computeTime(180 - 0.833, t[1]);

    double Dhuhr = computeMidDay(t[2]);
    double Asr = computeAsr(1.0 + getAsrJuristic(), t[3]);
    double Sunset = computeTime(0.833, t[4]);

    double Maghrib = computeTime(mParams[2], t[5]);
    double Isha = computeTime(mParams[4], t[6]);
  ...
  List<double> get mParams => methodParams[getCalcMethod()]!;
